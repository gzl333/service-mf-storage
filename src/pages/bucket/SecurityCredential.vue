...
const keysRow = computed(() => Object.values(store.tables.keyPairTable.byId))
              ...
              <template v-slot:body="props">
                <q-tr :props="props" :class="props.expand ? 'bg-blue-1':''">
                  ...
                  <q-td key="operation" :props="props">
                    <q-btn color="primary" unelevated no-caps @click="store.triggerCreateTokenDialog()">
                      {{ tc('创建新Token') }}
                    </q-btn>
                  </q-td>
                </q-tr>
    ...
    <div class="q-mt-md">
      <q-list bordered >
        ...
        >
          ...
          <q-card>
            <q-card-section>
              每个访问密钥包含access_key和secret_key密钥对，访问密钥不只是简单的身份认证，每次http请求前都需要根据请求的url的path和有效期时间戳重新计算auth_key字符串，此认证方式安全性高，使用稍微复杂一些(具体生成过程请参考帮助文档)。 auth_key的格式为“evhb-auth {access_key}:{hmac_sha1}:{data_base64}”，应包含在Authorization HTTP标头中，密钥应以字符串文字“evhb-auth”为前缀，空格分隔两个字符串。
            </q-card-section>
            <q-card-section class="text-primary">
              例如：Authorization: evhb-auth xxx:xxx:xxx
            </q-card-section>
            <q-card-section>
              <q-btn class="q-ma-sm" color="primary" no-caps unelevated @click="addKey">{{ tc('创建新访问密匙') }}</q-btn>
              <q-table
                class="rounded-borders"
                flat
                square
              ...
              >
                <template v-slot:body="props">
                  <q-tr :props="props" :class="props.expand ? 'bg-blue-1':''">
                    ...
                    <q-td key="operation" :props="props">
                      <q-btn color="primary" unelevated no-caps @click="store.triggerChangeKeyStateDialog(props.row.access_key, props.row.state)">
                        {{ props.row.state ? tc('停用') : tc('启用') }}
                      </q-btn>
                      <q-btn class="q-ml-xs" color="primary" unelevated no-caps @click="store.triggerDeleteKeyDialog(props.row.access_key)">
                        {{ tc('删除') }}
                      </q-btn>
                    </q-td>
                  </q-tr>
